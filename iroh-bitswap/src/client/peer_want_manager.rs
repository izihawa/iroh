...
impl PeerWantManager {
    ...
    pub async fn add_peer(&mut self, peer_queue: &MessageQueue, peer: &PeerId) {
        ...
        info!("add_peer: {:?}, {:?}", peer, self.broadcast_wants);

        self.peer_wants.insert(
            *peer,
            PeerWant {
                want_blocks: Default::default(),
                want_haves: self.broadcast_wants.clone(),
            },
        );
        for cid in self.broadcast_wants.iter() {
        ...
        if !self.broadcast_wants.is_empty() {
            peer_queue
                .add_broadcast_want_haves(&self.broadcast_wants)
                .await;
        }
    }
    ...
    pub(super) async fn send_cancels(
        ...
        self.broadcast_wants =
            &self.broadcast_wants - &AHashSet::from_iter(cancels.iter().cloned());
